...
*/		
		publicSunrise = s2;
		birkutHashahar = s2 - 1; // 42 regular minutes before sunrise -- ברכות השחר
		patachEliyaou = s2 - 56/60;// =  timeadj(s2 - 38/60, ampm); // 38 regular minutes before sunrise -- פתח אליהו
		korbanot = s2 - 53/60;// = 		 timeadj(s2 - 35/60, ampm);	// 35 regular minutes before sunrise -- קורבנות
		psokeiDzimra = s2 - 40/60;// =   timeadj(s2 - 22/60, ampm); // 22 regular minutes before sunrise -- פסוקי דזמרה
		nishmat = s2 - 15/60;
		yozerOr = s2 - 10/60;// =	     timeadj(s2 - 8/60, ampm);  // 08 regular minutes before sunrise -- יוצר אור
